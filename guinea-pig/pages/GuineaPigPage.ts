...
export default class GuineaPigPage {
  ...
  constructor(page: Page) {
    ...
    // Top text blocks
    this.title = page.getByRole(Constants.Roles.Heading, {
      level: 1,
      name: /Selenium sandbox/i,
    });
    ...
    this.exampleLink = page.getByRole(Constants.Roles.Link, {
      name: "i am a link",
    });
    // Comment area (labels are visible text on the page)
    this.yourCommentsLabel = page.getByText(Constants.Texts.YourComments, {
      exact: false,
    });
    // Client diagnostics
    this.clientTimeLabel = page.getByText(Constants.Texts.ClientTime, {
      exact: false,
    });
